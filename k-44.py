...
def start_worker(worker_id, code, worker_ids):
    ...
    logging.info(f"启动{worker_id}号, Worker ID: {worker_id_str}")
    print(f"{datetime.datetime.now().strftime('%Y-%m-%d|%H:%M:%S')} 🪄 启动{GREEN}{worker_id}{RESET}号, Worker ID: {GREEN}{worker_id_str}{RESET}")
...
def kill_worker(worker_id, worker_ids):
    ...
    try:
        ...
        logging.info(f"成功终止{worker_id}号, Worker ID: {worker_id_str}")
        print(f" 🔧 成功终止{YELLOW}{worker_id}{RESET}号, Worker ID: {YELLOW}{worker_id_str}{RESET}")
...
def main():
    ...
    while True:
        ...
        print(f"{datetime.datetime.now().strftime('%Y-%m-%d|%H:%M:%S')} 💻 {check_interval // 60}分钟推理量: {GREEN}{total_finish}{RESET} ")
